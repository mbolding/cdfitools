...
RE1 = [AEPI(:,slicestart:sliceend) AEPI(:,end/4+slicestart:end/4+sliceend) AEPI(:,end/2+slicestart:end/2+sliceend) AEPI(:,3*end/4+slicestart:3*end/4+sliceend)];
sprintf('size RE1 = %d, %d\n',size(RE1,1),size(RE1,2))
...
%
 for nc =  1:64 
    efac = (nc-32)*0.05;
    factor = (efac)*i;
    REz(:,2:2:end) = RE1(:,2:2:end)*exp(factor);
...
NC
    for sliceno=1:nslices
        ...
        AE1(1:end/4,2:2:end)         = AE1(1:end/4,2:2:end)*exp(newfc);
        AE1(end/4+1:end/2,2:2:end)   = AE1(end/4+1:end/2,2:2:end)*exp(newfc);
        AE1(end/2+1:3*end/4,2:2:end) = AE1(end/2+1:3*end/4,2:2:end)*exp(newfc);
        AE1(3*end/4+1:end,2:2:end)   = AE1(3*end/4+1:end,2:2:end)*exp(newfc);
