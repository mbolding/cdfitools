...
//
  ...
  function startElement($p, $n, $a)  // parser, name ,attrs
  {
    global $fulldir;
    global $fullstudyid;
    if ($n != "ACTION")
    ...
    list($dir,$protocol,$runno) = split('[/_]', $a['DATA']);  // e.g.: data / epidw _ 01
    ...
    $procparname = $fulldir."/".$a['DATA'].".fid/procpar";
  ...
  if (!($fp = fopen($studyxml,"r")))  {
    echo "ERROR: could not open XML input\n"; die; 
  }
  while ($data = fread($fp, 4096)) {
    if (!xml_parse($xml_parser, $data, feof($fp))) {
      echo "ERROR: xml parse: ".xml_error_string(xml_get_error_code($xml_parser)); die;
    }
  }
