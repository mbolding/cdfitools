...
%     response.secs=zeros(1,nframes);
    ...
    Screen('Flip', w,[],[],[],1);
    Screen('FillOval', w, fix_color, fix_cord);	% draw fixation dot (flip erases it)
    Screen('FillOval', w2, fix_color, fix_cord);	% draw fixation dot (flip erases it)
    Screen('Flip', w,[],[],[],1);
    
    disp('Press shift key to begin')
    while 1
        % scan the keyboard for experimentor input
        exKeyIsDown = KbCheck(devices.keyInputInternal);
        if(exKeyIsDown)
...
%         if mod(ceil(i/(fps*period*TR/2)),2)==1
    ...
    %         r_out = find(r > rmax | r < rmin);	% dots to reposition
        ...
        Screen('DrawDots', w2, xymatrix, s, colvect, center,1);  % change 1 to 0 to draw square dots
%         if (doublebuffer==1)
            Screen('Flip', w,[],[],1);
            Screen('Flip', w2);
%         end;
...
%         end;
        ...
        % scan the keyboard for experimentor input
        [exKeyIsDown,~,exKeyCode] = KbCheck(devices.keyInputInternal);
        if(exKeyIsDown)
