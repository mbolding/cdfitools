...
try
    ...
    spriteSize = spriteSizeDeg/degPerPixel; % The height and width of the sprite in pixels (the sprite is square)
    
    crossThickness = crossThicknessDeg/degPerPixel;
    numberOfSpriteFrames = 1; % The number of animation frames for our sprite   
    amplitudeX = amplitudeXdeg / degPerPixel;
    Period = 4 * amplitudeXdeg / averageDegPerSec ;
    % Period = 10; % period of sin in seconds
    ...
    for i = 1 : numberOfSpriteFrames
        ...
        % consists of noise.        
        spriteFrame(i) = Screen('MakeTexture', window, crossMask .* whiteColor);
    end
    ...
    while ~any(buttons)
        ...
        Screen('DrawText', window, 'Click to exit', 0, 0, whiteColor);
