...
app_main()
{
  ...
  */
  ...
  graphic_printf(target_foreground_color, target_background_color, 1, SCRWIDTH/2, 2*SCRHEIGHT/30,
                 "Start with [C]alibration and [V]erification; press [space] to start a session");
...
//SEAN - file i/o
  ...
  if(our_file_name[0])    // If file name set, open it
    { 
      sprintf(File_config, "%s%s", our_file_name, ".txt");
  ...
  eyecmd_printf("calibration_type = HV5");                  // Setup calibration type
